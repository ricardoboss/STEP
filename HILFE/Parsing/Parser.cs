...
public class Parser
{
    ...
    public async IAsyncEnumerable<Statement> ParseAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        while (tokenQueue.TryDequeue(out var token))
        {
            ...
            if (token.Type == TokenType.TypeName)
                yield return ParseVariableDeclaration(token);
            else if (token.Type == TokenType.Identifier)
                yield return ParseIdentifierUsage(token);
    ...
    private Statement ParseIdentifierUsage(Token token)
    {
        var nextType = tokenQueue.PeekType();
        if (nextType == TokenType.ExpressionOpener)
            return ParseFunctionCall(token);

        if (nextType.IsMathematicalOperation())
            return ParseMathematicalOperation(token);

        return ParseVariableAssignment(token);
    }

    private Statement ParseVariableAssignment(Token token)
    {
        ...
        var valueExpression = ExpressionParser.Parse(expressionTokens);

        return new VariableAssignmentStatement(token, valueExpression);
    }

    private Statement ParseMathematicalOperation(Token token)
    {
        ...
        var expression = ExpressionParser.Parse(new []
        {
            token, operationToken,
        }.Concat(valueExpTokens).ToArray());

        return new VariableAssignmentStatement(token, expression);
    }

    private Statement ParseFunctionCall(Token token)
    {
        ...
        return new FunctionCallStatement(token, args);
    }

    private Statement ParseVariableDeclaration(Token token)
    {
        ...
        return new VariableDeclarationStatement(token, identifier, expression);
    }
