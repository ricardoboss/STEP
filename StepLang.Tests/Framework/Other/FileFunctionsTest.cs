...
public class FileFunctionsTest
{
    ...
    public void TestFileFunctions(string platform, string filename)
    {
        ...
        var preWriteExistsResult = fileExistsFunction.Invoke(new TokenLocation(), interpreter, new List<ExpressionNode> { LiteralExpressionNode.FromString(filename) });
        ...
        var postWriteExistsResult = fileExistsFunction.Invoke(new TokenLocation(), interpreter, new List<ExpressionNode> { LiteralExpressionNode.FromString(filename) });

        var postWriteExistsBoolResult = Assert.IsType<BoolResult>(postWriteExistsResult);
        Assert.True(postWriteExistsBoolResult.Value);

        var firstReadResult = fileReadFunction.Invoke(new TokenLocation(), interpreter, new List<ExpressionNode> { LiteralExpressionNode.FromString(filename) });
        ...
        var secondReadResult = fileReadFunction.Invoke(new TokenLocation(), interpreter, new List<ExpressionNode> { LiteralExpressionNode.FromString(filename) });
        ...
        var deleteResult = fileDeleteFunction.Invoke(new TokenLocation(), interpreter, new List<ExpressionNode> { LiteralExpressionNode.FromString(filename) });
