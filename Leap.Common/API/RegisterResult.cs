...
public record RegisterResult(string Code, string Message, string? Token)
{
    ...
    public static RegisterResult Invalid(string message) => new("invalid", message, null);

    public static RegisterResult Success(string token) => new("success", "User created successfully", token);
}