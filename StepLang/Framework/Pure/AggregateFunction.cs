...
public class AggregateFunction : GenericFunction<ExpressionResult, ListResult, FunctionResult>
{
        ...
        ListResult argument2, FunctionResult argument3)
    {
        ...
        if (aggregator.Parameters.Count != 2)
            throw new InvalidArgumentTypeException(callLocation, $"Callback function must have 2 parameters, but has {aggregator.Parameters.Count}");

        var accumulatorParameter = aggregator.Parameters[0];
        if (!accumulatorParameter.HasResultType(initial.ResultType))
            throw new InvalidArgumentTypeException(callLocation, $"First parameter of callback function must accept initial value type {initial.ResultType}, but is {accumulatorParameter.ResultTypesToString()}");
